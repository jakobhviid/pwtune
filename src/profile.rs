...
/// Repo/data root: $SPEAKER_EQ_HOME if set (the justfile exports it), else CWD.
pub fn home() -> PathBuf {
    std::env::var("SPEAKER_EQ_HOME")
        .map(PathBuf::from)
...
pub const DEPLOY_PREFIX: &str = "eqlab-";
...
pub fn write_profile(name: &str, bands: &[Band], boost: f64, target: &str, out: &PathBuf) -> Result<()> {
    ...
    let header = format!(
        "# {name} — PipeWire filter-chain speaker EQ\n\
         # Auto-generated by speaker-eq; edit freely and re-measure to refine.\n\
         # Makeup gain (preamp): +{boost} dB   |   {} EQ band(s)\n\
         #\n\
         # target-match: {target}\n\
         #   `speaker-eq install` resolves this to the actual sink node.name (it\n\
